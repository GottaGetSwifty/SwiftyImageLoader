    ...
    */
                             ...
                             completion: ImageCompletion? = nil)
    {
        ...
        guard let urlAbsoluteString = request.url?.absoluteString else {
            self.completionHolder.completion?(false, CachedURLResponse.improperUrl)
            return
        }
        ...
        if let existingImage = cacheManager.image(for: request) {

        }
        cacheManager.loadImage(for: request)
